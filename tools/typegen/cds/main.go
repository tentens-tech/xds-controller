...
// Package main generates CDS (Cluster Discovery Service) types from go-control-plane.
package main
...
func buildTypeRegistry() map[string]reflect.Type {
	...
	typesToScan := []interface{}{
		...
		core.Http1ProtocolOptions_HeaderKeyFormat{},
		core.Http1ProtocolOptions_HeaderKeyFormat_ProperCaseWords{},
		core.Http2ProtocolOptions{},
		core.HttpProtocolOptions{},
		core.UpstreamHttpProtocolOptions{},
		// Cluster package types
		cluster.Cluster{},
		cluster.Cluster_RingHashLbConfig{},
		cluster.Cluster_MaglevLbConfig{},
		cluster.Cluster_OriginalDstLbConfig{},
		cluster.Cluster_LeastRequestLbConfig{},
		cluster.Cluster_RoundRobinLbConfig{},
		cluster.Cluster_CommonLbConfig{},
		cluster.Cluster_CommonLbConfig_ZoneAwareLbConfig{},
		cluster.Cluster_CommonLbConfig_LocalityWeightedLbConfig{},
		cluster.Cluster_CustomClusterType{},
		cluster.Cluster_EdsClusterConfig{},
		cluster.Cluster_LbSubsetConfig{},
		cluster.Cluster_SlowStartConfig{},
		cluster.Cluster_PreconnectPolicy{},
		cluster.Cluster_TransportSocketMatch{},
		cluster.CircuitBreakers{},
		cluster.CircuitBreakers_Thresholds{},
		cluster.OutlierDetection{},
		cluster.UpstreamConnectionOptions{},
...
var (
	outputDir = flag.String("output", "pkg/xds/types/cds", "Output directory")
...
func main() {
	...
	fmt.Printf("Successfully generated CDS types in %s\n", *outputDir)
}
...
func (g *Generator) Generate() error {
	...
	// Start with the Cluster type
	clusterType := reflect.TypeOf(cluster.Cluster{})
	g.queueType(clusterType, "CDS")
	...
	// Post-process: replace undefined types with RawExtension
	g.replaceUndefinedTypes()
...
// replaceUndefinedTypes finds references to types that weren't generated and replaces them with RawExtension
func (g *Generator) replaceUndefinedTypes() {
	g.imports["k8s.io/apimachinery/pkg/runtime"] = "runtime"

	// Build a set of defined types (including enums)
	...
	// Check all field types for undefined references
	for _, typeDef := range g.typeDefs {
		for i, field := range typeDef.Fields {
			newType := g.replaceUndefinedTypeRef(field.Type, definedTypes)
			if newType != field.Type {
				if g.verbose {
					fmt.Printf("Replacing undefined type %s.%s: %s -> %s\n", typeDef.Name, field.Name, field.Type, newType)
				}
				typeDef.Fields[i].Type = newType
			}
		}
...
// replaceUndefinedTypeRef checks if a type reference is to an undefined type and replaces it
func (g *Generator) replaceUndefinedTypeRef(fieldType string, definedTypes map[string]bool) string {
	// Handle map types: map[key]value
	if strings.HasPrefix(fieldType, "map[") {
		// Extract key and value types
		inner := strings.TrimPrefix(fieldType, "map[")
		bracketIdx := strings.Index(inner, "]")
		if bracketIdx == -1 {
			return fieldType
		}
		keyType := inner[:bracketIdx]
		valType := inner[bracketIdx+1:]

		// Check if value type needs replacement
		newValType := g.replaceUndefinedTypeRef(valType, definedTypes)
		if newValType != valType {
			return fmt.Sprintf("map[%s]%s", keyType, newValType)
		}
		return fieldType
	}
	...
	if strings.HasPrefix(fieldType, "[]*") {
		inner := strings.TrimPrefix(fieldType, "[]*")
		if !g.isBuiltinOrDefined(inner, definedTypes) {
			return "[]*runtime.RawExtension"
		}
		return fieldType
	}
...
// isBuiltinOrDefined checks if a type is a builtin type or defined in the generated output
func (g *Generator) isBuiltinOrDefined(typeName string, definedTypes map[string]bool) bool {
	// Builtin types
	switch typeName {
	...
	// Check if defined in our generated types
...
func (g *Generator) processType(t reflect.Type, outputName string) {
	...
	for i := 0; i < t.NumField(); i++ {
		...
		if fieldType == "" {
			if g.verbose {
				fmt.Printf("  Skipping field %s (unsupported type %v)\n", field.Name, field.Type)
			}
			continue
		}
...
func (g *Generator) writeOutput() error {
	...
	buf.WriteString("// Code generated by tools/typegen/cds. DO NOT EDIT.\n")
	buf.WriteString("// Source: github.com/envoyproxy/go-control-plane/envoy/config/cluster/v3\n")
	buf.WriteString(fmt.Sprintf("// Envoy version: %s\n", g.envoyTag))
	buf.WriteString("// To regenerate: make generate-cds\n\n")
	buf.WriteString("package cds\n\n")
	...
	for i, name := range typeNames {
		if name == "CDS" {
			typeNames = append([]string{"CDS"}, append(typeNames[:i], typeNames[i+1:]...)...)
			break
		}
	}
	...
	filename := filepath.Join(g.outputDir, "cds_generated.go")

...
// Package main generates EDS (Endpoint Discovery Service) types from go-control-plane.
package main
...
// This allows dynamic type lookup when processing oneofs
func buildTypeRegistry() map[string]reflect.Type {
	registry := make(map[string]reflect.Type)

	// Register types from imported packages by reflecting on known entry points
	// and recursively discovering all related types
	typesToScan := []interface{}{
		...
		core.HealthCheck{},
		core.HealthCheck_HttpHealthCheck{},
		core.HealthCheck_TcpHealthCheck{},
		core.HealthCheck_GrpcHealthCheck{},
		core.HealthCheck_CustomHealthCheck{},
		core.HealthCheck_Payload{},
		core.HealthCheck_TlsOptions{},
		core.GrpcService{},
		...
		core.WatchedDirectory{},
		// Endpoint package types
		endpoint.ClusterLoadAssignment{},
		endpoint.ClusterLoadAssignment_Policy{},
		endpoint.ClusterLoadAssignment_Policy_DropOverload{},
		endpoint.LocalityLbEndpoints{},
		endpoint.LocalityLbEndpoints_LbEndpointList{},
		endpoint.LbEndpoint{},
		endpoint.Endpoint{},
		endpoint.Endpoint_HealthCheckConfig{},
		endpoint.Endpoint_AdditionalAddress{},
		endpoint.LedsClusterLocalityConfig{},
	}
...
var (
	outputDir = flag.String("output", "pkg/xds/types/eds", "Output directory")
...
func main() {
	...
	fmt.Printf("Successfully generated EDS types in %s\n", *outputDir)
}
...
type Generator struct {
	...
	typeRegistry   map[string]reflect.Type // Dynamic type registry
}
...
func (g *Generator) Generate() error {
	...
	// Start with the ClusterLoadAssignment type (main EDS type)
	edsType := reflect.TypeOf(endpoint.ClusterLoadAssignment{})
	g.queueType(edsType, "EDS")

	// Process all queued types - the queue will grow as we discover more types
	...
	// Post-process: replace recursive types with RawExtension to break cycles
	g.replaceRecursiveTypes()

	return g.writeOutput()
}

// replaceEmptyTypes finds empty structs and replaces references to them with *runtime.RawExtension
func (g *Generator) replaceEmptyTypes() {
	...
	// Replace references to empty types in all type definitions
	...
	// Remove empty type definitions
	for name := range emptyTypes {
		delete(g.typeDefs, name)
	}
}

// replaceEmptyTypeRef replaces references to empty types with *runtime.RawExtension
func (g *Generator) replaceEmptyTypeRef(fieldType string, emptyTypes map[string]bool) string {
	// Check for pointer to empty type
	...
	// Check for slice of pointer to empty type
	if strings.HasPrefix(fieldType, "[]*") {
		inner := strings.TrimPrefix(fieldType, "[]*")
		if emptyTypes[inner] {
			return "[]*runtime.RawExtension"
...
// replaceRecursiveTypes finds recursive type references and replaces them with RawExtension
func (g *Generator) replaceRecursiveTypes() {
	g.imports["k8s.io/apimachinery/pkg/runtime"] = "runtime"

	// For each type, check if any of its fields create recursive references
...
func (g *Generator) fetchEnumDefinitions() {
	// Track fetched packages to avoid duplicates
...
// discoverPackages recursively discovers all envoy packages referenced by a type
func (g *Generator) discoverPackages(t reflect.Type, seen map[string]bool) {
	...
	// Extract the envoy subpath (e.g., "envoy/config/endpoint/v3")
	if idx := strings.Index(pkgPath, "envoy/"); idx != -1 {
		subPath := pkgPath[idx:]
		if !seen[subPath] {
			seen[subPath] = true

			// Recursively discover packages from fields
...
// fetchPackageEnums fetches all .pb.go files from a package directory
...
type pendingOneofField struct {
	wrapperKey string // e.g., "RedirectAction_PathRedirect"
	fieldName  string
	fieldType  string
	jsonName   string
}
...
func (g *Generator) parseEnumsFromReader(r io.Reader) error {
	...
	for scanner.Scan() {
		...
		// Parse interface implementation methods like: func (*RedirectAction_PathRedirect) isRedirectAction_PathRewriteSpecifier() {}
	...
	for scanner.Scan() {
		...
		if inOneofWrapper {
			// Check for struct closing brace - must be just "}" with optional whitespace, not inside a comment
	...
	for _, pf := range pendingFields {
		...
		if !hasExactMatch {
			...
			// Extract parent type from wrapper key
...
// queueDiscoveredType tries to queue a type discovered from oneof parsing
func (g *Generator) queueDiscoveredType(fieldType string) {
	// Extract the type name (remove pointer prefix and package qualifier)
	typeName := strings.TrimPrefix(fieldType, "*")
	if idx := strings.LastIndex(typeName, "."); idx >= 0 {
		typeName = typeName[idx+1:]
	}

	// Look up in our type registry
...
func (g *Generator) processType(t reflect.Type, outputName string) {
	...
	// Check if the struct already has a TypedConfig field
	...
	// Check if this type needs an explicit TypedConfig field (only if not already present)
	...
	for i := 0; i < t.NumField(); i++ {
		...
		// Handle oneof interface fields
		if field.Type.Kind() == reflect.Interface {
			interfaceTypeName := field.Type.Name()

			// Check if we have dynamically discovered wrapper definitions for this oneof
			if options, ok := g.oneofWrappers[interfaceTypeName]; ok && len(options) > 0 {
				...
				// Add each concrete option as a field (skip duplicates)
				for _, opt := range options {
					...
					// Always simplify oneof types - handles qualified types and wrapper types
					fieldType = g.simplifyOneofType(fieldType)
				...
				// For config_type oneofs, add TypedConfig for dynamic extension
				g.imports["k8s.io/apimachinery/pkg/runtime"] = "runtime"
		...
		// Skip self-referential fields (e.g., type containing *SameType)
...
// isConfigTypeOneof checks if a field name represents a oneof that should have typed_config
...
// simplifyOneofType converts a qualified type to a local type reference
func (g *Generator) simplifyOneofType(fieldType string) string {
	...
	// Handle slice types (e.g., []byte)
	if strings.HasPrefix(fieldType, "[]") {
		return fieldType
	}

	// Check if this is an enum type (already defined in enumDefs)
	if _, isEnum := g.enumDefs[fieldType]; isEnum {
		return fieldType
	}

	// Check if this looks like a protobuf oneof wrapper type (Parent_Option format)
	// These are generated by protobuf and should be treated as RawExtension
	if strings.Contains(fieldType, "_") && isPointer {
		parts := strings.Split(fieldType, "_")
		if len(parts) >= 2 {
			// This looks like a wrapper type (e.g., RegexMatcher_GoogleRE2)
			g.imports["k8s.io/apimachinery/pkg/runtime"] = "runtime"
			return "*runtime.RawExtension"
		}
	}

	// Check if this looks like an undiscovered enum type
...
// typesNeedingTypedConfig returns true for types that need an explicit TypedConfig field
...
func (g *Generator) writeOutput() error {
	...
	buf.WriteString("// Code generated by tools/typegen/eds. DO NOT EDIT.\n")
	buf.WriteString("// Source: github.com/envoyproxy/go-control-plane/envoy/config/endpoint/v3\n")
	buf.WriteString(fmt.Sprintf("// Envoy version: %s\n", g.envoyTag))
	buf.WriteString("// To regenerate: make generate-eds\n\n")
	buf.WriteString("package eds\n\n")
	...
	for i, name := range typeNames {
		if name == "EDS" {
			typeNames = append([]string{"EDS"}, append(typeNames[:i], typeNames[i+1:]...)...)
			break
		}
	}
	...
	filename := filepath.Join(g.outputDir, "eds_generated.go")
...
func (g *Generator) writeStruct(buf *strings.Builder, t *TypeDef) {
	...
	for _, f := range t.Fields {
		// Add kubebuilder marker for RawExtension fields to preserve @type and other dynamic fields
...
// isRecursiveField checks if a field type could create recursive references
func (g *Generator) isRecursiveField(parentType, fieldType string) bool {
	// Check for direct self-reference (e.g., *SameType in SameType)
...
// checkRecursiveReference checks if elemType has any reference back to parentType
func (g *Generator) checkRecursiveReference(parentType, elemType string) bool {
	// Check if the element type exists in our type definitions
	elemDef, exists := g.typeDefs[elemType]
	if !exists {
		return false
	}

	// Check if any field of the element type references back to the parent or creates a cycle
	for _, f := range elemDef.Fields {
		// Check for direct reference back to parent
		if strings.Contains(f.Type, "*"+parentType) || f.Type == parentType {
			return true
		}
		// Check for indirect references via other types (one level deep)
